(ns ongakuka.worker
  ...
  ここには判断を置かない —— どのハンドラが答えるかは `ongakuka.route/dispatch`
  が決め、ページの中身は `ongakuka.view` が組む。どちらも `.cljc` なので、
  ブラウザもビルドも無しにテストできる。
  ...
  (:require [ongakuka.route :as route]
...
(defn fetch-handler [req env _ctx]
  (let [url (js/URL. (.-url req))
    ...
    (case action
      ...
      :health (json {:ok true :app "ongakuka" :runtime "cljs"
